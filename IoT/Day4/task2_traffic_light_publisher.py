...
while True:
    ...
    if (cond == 1):
        ...
        GPIO.output(waitLED, 0)
        mqtt.publish(topic_state, 1, qos=0, retain=False)   #mqtt state 1 sending
        time.sleep(5)
        for i in range(3):
            mqtt.publish(topic_state, 2, qos=0, retain=False)  
        ...
        mqtt.publish(topic_state, 3, qos=0, retain=False)  
        x = True
        cond = 0
